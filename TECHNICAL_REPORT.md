...
- **Whole-clip decoding of long test clips (final-day fix).** The pipeline originally cut audio into hard 38-second chunks decoded independently — slicing words mid-syllable and restarting the LM context at every boundary. 4,500 test clips (10.8 % of clips, **40 % of the test audio**; Somali most exposed) crossed that limit. The fix: encode clips > 38 s in **one pass** (the Conformer's relative position encodings extrapolate well beyond the 40 s training window) and beam-decode once with the full-clip LM context. Validated against references before deployment: **−0.87 macro-WER** on 60–110 s clips (5 languages improve, none regress), confirmed on natural > 38 s clips. Applied surgically to the long clips only ([`code/omni_kenlm_whole.py`](code/omni_kenlm_whole.py) + [`code/splice_qc.py`](code/splice_qc.py) with per-clip guards). A first variant (silence-point cutting + logits stitching) **failed** the same reference validation (Somali +5.3) and was discarded — the validation harness exists precisely for this.
- **Consensus post-processing (explored, not deployed).** A cross-deck exact-agreement vote corroborated by greedy witnesses flagged 478 short clips where the primary deck diverged alone ([`code/consensus_vote.py`](code/consensus_vote.py); worst case bounded at +0.0007, estimated word precision 60–75 %). We chose **not** to include it in the final submission: its evidence (inter-decode agreement) is weaker than the reference-level validation backing the whole-clip fix, and the final card keeps only reference-validated changes.
