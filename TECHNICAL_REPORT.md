...
- **Consensus post-processing (final submission).** For clips ≤ 38 s where two decodes from different parameter families produced the *exact same* transcription, corroborated by the greedy witness, while the primary deck diverged alone, the consensus transcription is taken (478 clips; rule and adversarial verification in [`code/consensus_vote.py`](code/consensus_vote.py); worst-case impact mathematically bounded at +0.0007).
